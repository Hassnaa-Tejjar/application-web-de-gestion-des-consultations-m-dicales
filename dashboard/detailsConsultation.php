...
try {
  ...
  $connexionDB = new PDO("mysql:host=localhost;dbname=db_healthcare", "root", ""); 
   $requete ="SELECT  patient.Nom_patient,patient.Prenom_patient,patient.Antecedents_medicaux,patient.Antecedents_chirurgiicaux,consultation.Date_consultation,consultation.Id_consultation,consultation.Symptome,consultation.Id_consultation,consultation.Poids,consultation.Id_patient,consultation.Maladie,consultation.Glycemie,consultation.Temperature,consultation.Taille,consultation.Pression_arterielle,consultation.Frequence_cardiaque,consultation.Diagnostique_medicale,consultation.Symptome FROM consultation  INNER JOIN patient ON consultation.Id_patient =patient.Id_patient  where Id_consultation = '".$_GET["idp"]."'";
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content" >
            ...
            <div class="modal-header"style="background-color:#E74C3C;width:650px;font-size 25px;height:140px;">
                ...
                <h2 class="modal-title" style="margin-left:70px;margin-bottom:-78px;" > <img src="image\immg13.png" width="140" style="margin-bottom:-45px;" ><strong style="marginleft:-15px;margin-top:40px;font-size:50px"> Fiche Consultation</strong></h2>
            </div>
            <div class="modal-body">
                ...
                <form method="post" id="insert_form" action="modifierCons.php">
                    ...
                    <input name="idcons" id="idcons" class="form-control" value="<?php $requete=$connexionDB->query("SELECT count(patient.Nom_patient)   from consultation INNER JOIN patient ON consultation.Id_patient =patient.Id_patient  WHERE patient.Nom_patient ="."'".$data['Nom_patient']."'"." AND Date_consultation <= "."'".$data['Date_consultation']."'"." ORDER BY Date_consultation")->fetch(); echo 'consultation N°: '.reset($requete);?>" ></th>
                         
                         <th>
                    <label style="position:relative;left:78px;" >Date Consultation (*)</label><br/>
